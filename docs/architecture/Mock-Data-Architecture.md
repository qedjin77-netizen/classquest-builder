...
- `src/lib/data/`와 `src/lib/mock/`(fixtures)은 **신규 폴더**다 — 2026-08-03 승인됨 (계획서 §11.1).
...
- Supabase 연결 시 이 함수들의 구현이 서버 호출로 교체되고, fixtures의 정답 필드는 서버 시드로 이동한다. CLAUDE.md의 최종 기준은 유지된다.
- **이 임시 예외는 2026-08-03 승인되었으며 mock 단계에만 적용한다** (계획서 §11.1).
